...
auc_cal_cv_tt <- function(single_one,all_data_train,data_test,y_surv_train,y_surv_test,nfold=10,seed=NULL) {
  # em<-ensemble_model(t(single_one),all_data_combat[,c(grep('TCGA',colnames(all_data_combat)),grep('GSM',colnames(all_data_combat))[1:50])],
  #                    y_surv_train)
  set.seed(seed)
  ...
  auc_eNet_cv<-auc_roc(y_surv = y_surv_train,survival_predictions['lasso',])
